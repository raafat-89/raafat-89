...
app.listen(PORT, () => {
    ...
    bot.sendMessage(CONFIG.TELEGRAM_CHAT_ID, '✅ **النسخة النهائية والمحسّنة بدأت العمل!**').catch(err => console.error(err.message));
});
...
async function mainMonitoringLoop() {
    ...
    for (const fiat of CONFIG.FIAT_CURRENCIES) {
        for (const crypto of CONFIG.CRYPTO_ASSETS) {
            ...
            await new Promise(resolve => setTimeout(resolve, 1000)); // فاصل بسيط
        }
    }
...
async function sendStatusReport() {
    ...
    if (reportData.errors.length > 0) {
        reportMessage += `\n**⚠️ مشاكل واجهتني:**\n`;
        const errorsToShow = [...new Set(reportData.errors)]; // إزالة الأخطاء المكررة
