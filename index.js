...
// تفعيل polling ليستمع البوت للرسائل
...
app.get("/", (req, res) => res.send('P2P Bot On-Demand Reports is alive!'));
app.listen(PORT, () => {
    console.log(`Server running on port ${PORT}`);
    safeSendMessage('✅ **النسخة النهائية (مع التقرير عند الطلب) بدأت العمل!**\n\nأرسل `/report` للحصول على تقرير الحالة.');
});

// --- الأمر الجديد للحصول على تقرير ---
bot.onText(/\/report/, (msg) => {
    // التأكد من أن الأمر قادم منك فقط
...
async function mainMonitoringLoop() {
    try {
        ...
        if (!allPrices) {
            console.log("Price fetch failed, report will show error.");
            return; // لا نوقف الحلقة، فقط ننتظر الدورة التالية
        }
