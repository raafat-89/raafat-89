...
// --- الإعدادات الرئيسية القابلة للتعديل ---
const CONFIG = {
...
async function safeSendMessage(message) {
    try { await bot.sendMessage(CONFIG.TELEGRAM_CHAT_ID, message, { parse_mode: 'Markdown' }); }
    catch (error) { console.error("Telegram send error:", error.message); }
}

app.get("/", (req, res) => res.send('P2P Ultimate Bot is alive!'));
app.listen(PORT, () => {
    console.log(`Server running on port ${PORT}`);
    safeSendMessage(`✅ **البوت النهائي بدأ العمل!**\nأرسل /report للحصول على تقرير الحالة.`);
});
...
async function processAndSendAlert(ad, platform, asset, fiat, marketPrice) {
    ...
    if (discount >= CONFIG.REQUIRED_DISCOUNT) {
        ...
        let adLink = `https://p2p.binance.com/en/advertiserDetail?advertiserNo=${ad.advertiserNo}`; // Default for Binance
